...
async def starting_commands(text_input, user_id_info, client):
    ...
    #start a new campaign
        ...
        await game_screen.print_credits(text_input, False, client)
        voice_channel = discord.utils.get(text_input.guild.voice_channels, name="the-catacombs-music")
...
async def move_player(direction, player_name, raw_input, client):
    ...
    if encounter_char == "#":
        ...
        if new_level < 10:
            ...
            while map_created_properly:
                map_created_properly = await generate_map(raw_input, new_level)
            voice = discord.utils.get(raw_input.guild.voice_channels, name="the-catacombs-music")
            voice.stop()
            music_val = random.randint(1,15)
            dungeon_song = "./music/dungeon_" + str(music_val) + ".mp3"
            voice.play(discord.FFmpegPCMAudio(dungeon_song))
